            ...
            <div class="card-body">
                ...
                <div class="current-doc-info" style="margin-bottom: 24px;">
                    <div class="doc-info-row">
                        ...
                        <span class="doc-info-value" style="font-size:1.2rem;font-weight:700;color:var(--primary);">${{ number_format($terreno->precio, 2) }} USD</span>
                    </div>
                    <div class="doc-info-row">
                    ...
                    <div class="doc-info-row">
                        ...
                        <span class="doc-info-value">{{ \Carbon\Carbon::parse($terreno->creado_en)->timezone('America/La_Paz')->translatedFormat('d \d\e F \d\e Y, H:i') }}</span>
                    </div>
                ...
                <div class="action-panel">
                    ...
                    <div class="action-buttons-lg">
                        <form action="{{ route('admin.procesar_terreno') }}" method="POST" style="display:inline;" id="approveTerreno">
                            @csrf
                            ...
                            <button type="submit" class="btn btn-success btn-lg" onclick="return confirm('¿Confirma la aprobación de este terreno para publicación?')">
                                ✅ Aprobar Terreno
                            </button>
                        </form>
                        <button type="button" class="btn btn-danger btn-lg" onclick="document.getElementById('rejectTerrenoSection').style.display='block'; this.style.display='none';" id="showRejectTerrenoBtn">
                            ❌ Rechazar Terreno
                        </button>
                    </div>
                    ...
                    <div id="rejectTerrenoSection" style="display:none; margin-top: 16px;">
                        <form action="{{ route('admin.procesar_terreno') }}" method="POST" id="rejectTerrenoForm">
                            ...
                            <div class="form-group">
                                <label for="rejectTerrenoComment" class="form-label">Motivo del rechazo <span class="required">*</span></label>
                                <textarea name="observacion" id="rejectTerrenoComment" class="form-control" rows="3" placeholder="Explique el motivo del rechazo del terreno..." required minlength="10"></textarea>
                                <small class="form-hint">Esta observación quedará registrada en el sistema.</small>
                            </div>
                            <div class="action-buttons">
                                <button type="button" class="btn btn-secondary" onclick="document.getElementById('rejectTerrenoSection').style.display='none'; document.getElementById('showRejectTerrenoBtn').style.display='inline-flex';">Cancelar</button>
            ...
            <div class="card-body">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px;">
                    ...
                    <div style="border-radius: var(--border-radius-sm); overflow: hidden; border: 1px solid rgba(0,0,0,0.08); box-shadow: var(--shadow-sm); transition: var(--transition); cursor: pointer; position: relative;" 
                         <img src="{{ $img->url }}" alt="Imagen terreno {{ $img->orden }}">
                        ...
                        <img src="{{ Storage::url(str_replace('/storage/', '', $img->ruta_archivo)) }}" alt="Imagen terreno {{ $img->orden }}">
...
<div class="modal-overlay" id="modalImagen" style="display:none;">
    <div class="modal" style="max-width: 90vw; max-height: 90vh; background: #000;">
        <div class="modal-header" style="border-bottom: 1px solid rgba(255,255,255,0.1);">
            ...
            <button class="modal-close" type="button" onclick="cerrarModal('modalImagen')" style="color: white;">&times;</button>
        </div>
        <div style="display: flex; align-items: center; justify-content: center; padding: 16px;">
            <img id="modalImagenSrc" src="" alt="Imagen ampliada" style="max-width: 100%; max-height: 75vh; object-fit: contain; border-radius: 8px;">
        </div>
    </div>
</div>
