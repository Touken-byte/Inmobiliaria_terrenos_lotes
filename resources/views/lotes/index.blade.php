...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-2">🟩 Lotes disponibles</h1>
        <p class="text-gray-600">Encuentra el lote ideal para tu próximo proyecto.</p>
    </div>

    <!-- Barra de Búsqueda GET -->
    <form action="{{ route('catalogo.lotes') }}" method="GET" class="mb-10 max-w-2xl">
    ...
    @if($terrenos->isEmpty())
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center max-w-2xl mx-auto mt-8">
            ...
            <p class="text-gray-500 mb-6">No encontramos lotes disponibles que coincidan con "<strong>{{ request('search') }}</strong>".</p>
            <a href="{{ route('catalogo.lotes') }}" class="inline-block bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium px-6 py-2.5 rounded-lg transition-colors">
                Limpiar Búsqueda
            </a>
        </div>
    @else
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div class="h-48 bg-gray-100 relative">
                        @if($terreno->imagenes->count() > 0)
                            <img src="{{ asset($terreno->imagenes->first()->ruta_archivo) }}" alt="Lote" class="w-full h-full object-cover">
                        @else
